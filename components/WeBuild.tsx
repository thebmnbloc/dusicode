      ...
      <div className="grid grid-cols-3 items-center justify-center gap-2">
        {sectors.map((sector, index) => (
          <div key={index} className="flex flex-col items-items justify-center bg-white hover:border-1 hover:border-orange-600 px-6 py-2 rounded-xl gap-4">
            <span className="bg-orange-100 text-orange-600 rounded-md w-12 h-12 flex items-center justify-center">{sector.icon}</span>
            ...
            <a className="text-sm text-orange-600">{sector.cta}</a>
          </div>
        ))}
      </div>
      
      <Button className="bg-orange-600 text-white hover:bg-orange-400 hover:cursor-pointer text-lg px-5 py-6 rounded-4xl">
